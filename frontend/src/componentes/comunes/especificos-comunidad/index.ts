// Exportaciones centralizadas de componentes específicos de comunidad
export { default as TarjetaPost } from './TarjetaPost';
export { default as TarjetaGrupo } from './TarjetaGrupo';
export { default as DashboardPrincipal } from './DashboardPrincipal';
// SistemaMensajeria eliminado
// SistemaMensajeriaConectado eliminado
export { default as NotificacionesPush } from './NotificacionesPush';
export { default as RedSocial } from './RedSocial';
export { default as Feed } from './Feed';
export { default as GruposMiembros } from './GruposMiembros';

// Re-exportar tipos si es necesario
export type { PropiedadesTarjetaPost } from './TarjetaPost';
export type { PropiedadesDashboardPrincipal } from './DashboardPrincipal';